...
- Defaults applied: addr `:8080`, base_url `https://api.anthropic.com`, version `2023-06-01`, timeout `1800s`.
- `timeout` bounds upstream **silence**, not total request time: it is applied as the transport's `ResponseHeaderTimeout` (wait for the first response byte) AND a mid-stream idle-read timeout (bytes stopped moving). There is **no** whole-request `http.Client.Timeout`, so a live stream (or slow non-stream generation) that keeps sending data runs unbounded and is never cut mid-response — only a genuinely silent/dead upstream (no first byte, or a stall longer than `timeout`) is dropped. Default `1800s` (30m) is generous so a slow non-stream generation, whose first byte only arrives once the whole answer is ready, still passes.
...
- **Full cloak (model-gated):** when the request model contains one of `claude-sonnet-5`, `claude-opus-4-8`, `claude-haiku-4-5`, OAuth requests get the aggressive cloak instead of the plain prefix: `system[]` is set to ONLY the Claude Code agent block, and the caller's original system content is relocated verbatim into the first user message inside a `<system-reminder>` block. Reason: Anthropic fingerprints `system[]` content of OAuth traffic — a third-party agent's system prompt (tool guidelines, custom identity) is billed to metered "extra usage" instead of the subscription plan (→ `400 "You're out of extra usage"` once credits are spent). Moving that content out of `system[]` makes the request classify as genuine Claude Code and ride the plan. Tools are NOT renamed — system relocation alone is empirically sufficient, so no response-side rewriting is needed. If there is no user message to host the reminder, it falls back to keeping the system content in place (agent block + system) so nothing is dropped. Gated to the model families actually routed through the subscription (targeted spoof, not a blanket rewrite).
- Known gaps: refreshed tokens are in-memory only (not persisted across restart); no reactive refresh on a 401 (only proactive-by-expiry); the full cloak is gated to the `claude-sonnet-5`/`claude-opus-4-8`/`claude-haiku-4-5` families (other models keep the minimal prefix and remain billable to extra usage); billing headers and the static Claude Code prompt are still not replicated.
**Verified:** `internal/provider/anthropic` (refresher + spoof + cloak table tests, gating) + `internal/server` refresh tests + live: real pi request body (opus system prompt, 4 tools, thinking) via cerber → `claude-sonnet-5` **200** (cloaked, on-plan), same body as `claude-opus-4-8` **400** (not gated) — 2026-07-10.
...
- **Tool calling (both dialects, both directions).** Request: `tools[].function` → Anthropic `tools[].input_schema` (a tool with no `parameters` gets `{"type":"object","properties":{}}`); `tool_choice` auto/none/required/`{type:function,name}` → auto/none/any/tool, and `parallel_tool_calls:false` → `disable_parallel_tool_use` (carried on an `auto` selector when no choice was given); an assistant turn's `tool_calls` → `tool_use` blocks with `arguments` parsed from its JSON string; `role:"tool"` messages → `tool_result` blocks on a user message, **consecutive ones merged into one message** (parallel calls). Response: `tool_use` blocks → `tool_calls` with `arguments` re-encoded as a JSON string, `content: null` on a tool-only turn. Streaming: `content_block_start`(tool_use) → a `tool_calls` opener carrying index/id/name, `input_json_delta` → argument fragments under the same index (OpenAI numbers tool calls among themselves, Anthropic numbers every block); a fragment for a block that never started is dropped.
- Tool errors are errors, never silent drops: a non-`function` tool type, a tool with no name, an unparseable `tool_choice`, a `tool` message with no `tool_call_id`, or `arguments` that are not valid JSON → 400 with a clear message. **Why:** a model told it has no tools answers "I cannot do that", which reads as a refusal — the failure this rule exists to prevent (an operator hit it 2026-08-18 and concluded claude-sonnet-5 "ignores tools").
**Verified:** `internal/translator` tests, 93.8% coverage — 2026-08-19; end-to-end through the server by `TestCompat_ToolsReachAnthropic`.

## Request compatibility mode (`X-Cerber-Compat`)
**What:** one OpenAI-compatible endpoint that either normalizes the corner cases every upstream disagrees on, or gets out of the way when the client would rather own them.
**DoD:**
- `POST /v1/chat/completions` resolves a mode per request: header `X-Cerber-Compat`, else `server.compat` from config, else `auto`. An unrecognized value → 400 (a typo surfaces instead of silently selecting the default); config validation rejects a bad `server.compat` at startup.
- `auto` — rewrites only what is deterministically known wrong for the **resolved target**: the `max_tokens`/`max_completion_tokens` spelling. Unknown parameters pass through, so a provider-specific extension keeps working without cerber learning about it first. Tool translation (above) applies in every mode that reaches a translating target.
- `raw` — the body reaches the upstream byte-for-byte. For a target needing dialect translation (`anthropic`, `gemini`) it is **400, not a silent downgrade**, and the message names `/v1/messages` and `compat=auto` as the ways forward. **Why:** raw promises byte-for-byte forwarding; translating anyway and still calling it raw would make the client's own corner-case handling unverifiable.
- `force` — `auto` plus coercion: parameters outside the OpenAI dialect are dropped rather than forwarded, an empty `tools` array is removed, and a `tool_choice`/`parallel_tool_calls` with no tools left is removed with it.
- **Learned token parameter.** The static guess is by model name (`gpt-5*`/`o1`/`o3`/`o4` → `max_completion_tokens`, everything else → `max_tokens`). In `auto`/`force`, a 400/422 whose body mentions either spelling triggers **exactly one** retry with the other one; on success the answer is remembered for that model, so an unclassified model self-corrects after one request. A retry that also fails relays the **original** error and learns nothing.
- An unrelated 400 is relayed to the client verbatim, body intact, with no retry.
- Documented in `/llm.md` and `/docs` (mode table + header table) — the same documents previously told agents to avoid the endpoint for tool calling.
**Verified:** `internal/compat` tests (98.0%) + `internal/server/compat_test.go` (auto rename both directions, raw forwarding, raw→400 on a translating target, unknown mode → 400, force dropping, retry+learn, unrelated 400, failed retry) — 2026-08-19.
...
- **1M context injection (OAuth, model-gated):** for OAuth requests whose model is 1M-capable (`claude-sonnet-5`, `claude-opus-4-8`), cerber injects `context-1m-2025-08-07` (deduped-merged). The 1M window is opt-in via this beta; Claude Code sends it itself, so non-Claude-Code clients (e.g. pi) would otherwise be capped at the default 200k and compact prematurely. Gated to 1M models — `claude-haiku-4-5` (200k) and all API-key requests are untouched (API-key clients control their own betas). NOTE: the client's own context manager still governs when it compacts — this only lifts the upstream ceiling to 1M; a client hard-coded to 200k must also raise its own context setting to benefit.
...
- `?days=N` (any positive integer; the dashboard offers 1/2/3/7/30) scopes totals/by_credential/by_model/by_client/series to the last N days, computed from hourly per-credential, per-model and per-client×model buckets (`usage.Tracker.SnapshotWindow`); omitted, zero, non-positive, or unparseable `days` falls back to the all-time snapshot.
...
- Hourly buckets (overall, per-credential, per-model, per-client×model) are retained ~30 days and persisted across restarts alongside the cumulative totals.
**Verified:** `internal/usage` (incl. `TestSnapshotWindow_*`, `TestSnapshotWindow_ByClientScopedToWindow`) + `internal/server` stats tests (incl. `TestStats_DaysWindow`) + live (`input 9/output 6` after one real call) — 2026-06-07, windowing (incl. by_client) added 2026-08-17.
...
- System messages → `systemInstruction`; user/assistant → `user`/`model`; text + base64(data:) images supported; http image URLs rejected (400).
- **Tool calling.** `tools[].function` → a single `tools[0].functionDeclarations` entry per tool; `tool_choice` auto/none/required/named → `toolConfig.functionCallingConfig` AUTO/NONE/ANY/ANY+`allowedFunctionNames`. Assistant `tool_calls` → `functionCall` parts (arguments string parsed to an object); `role:"tool"` messages → `functionResponse` parts on a user content, consecutive ones merged. Response/stream `functionCall` parts → OpenAI `tool_calls`, and `finish_reason` is corrected to `tool_calls` even though Gemini reports `STOP`.
- Gemini has no call ids, so cerber synthesizes `call_<n>_<function>`; on the next request the function name is recovered from the id, else from the message's `name`, else 400. **Why:** OpenAI keys a tool result by call id and Gemini keys it by function name, so the mapping has to survive the round trip through the client.
- Untranslatable request → 400; Gemini upstream errors relayed.
**Verified:** `internal/translator` Gemini tests (93.8% package) + `internal/provider/gemini` (92%) + live `make integration` (Gemini route → "pong" via real generativelanguage API) — Gemini tool translation 2026-08-19.
...
- **Host-level failover.** Configure hosts either the legacy way — `providers.ollama.base_url` + `fallback_base_urls` (optional, each http(s)) — or explicitly as `providers.ollama.hosts: [{base_url, concurrency}, …]` (first is primary, rest are ordered failover targets; takes precedence over base_url+fallback_base_urls). On a transport error or 5xx from one host, the same request (Chat/Forward/Images) is retried against the next (body rebuilt per attempt). A 4xx is returned as-is (client error, identical on every host) and not retried; with a single host a 5xx is still relayed unchanged. Same models must be pulled on each host. On firebat: gpu0 (GPU) primary, xeon (CPU) backup.
- **Per-host concurrency cap.** `providers.ollama.concurrency` (default 0 = unlimited; must be ≥ 0) is the default per-host in-flight cap; `hosts[].concurrency` overrides it per host (0 = inherit the provider default). A single local box (esp. one GPU) can only serve a few requests at once, so a burst of parallel embedding/chat workflows would otherwise overload it into 5xx/timeout. Each host holds its own semaphore; a slot is held for the whole request (incl. while the client streams the body) and released on body close, so requests beyond a host's cap queue instead of flooding it. A failed host attempt releases that host's slot before failing over. On firebat: gpu0 cap 2, xeon cap 1.
- **Capacity failure ≠ bad credential.** ollama is keyless (a dummy credential is injected so rotation works). A transport error / exhausted-host 5xx does **not** sideline that credential (`WithTransportPenaltyDisabled` → `provider.RotateOpts.PenalizeTransport=false`): such a failure is a capacity/host signal, not an auth failure, and sidelining the only credential would 503 the whole provider (`all upstream credentials are unavailable`) until the exponential backoff expired (up to 30 min) even after the box recovered. Instead the upstream error is relayed (502) and the credential stays usable, so the next request retries immediately. Only a genuine 401/403/429 sidelines a credential.
**Verified:** `internal/config` (defaults/no-creds/bad-url/fallback-url/hosts+concurrency validation/`ResolvedHosts` table) + `internal/provider/openai` failover tests (transport→2nd host, 5xx→2nd host, 4xx-not-retried, all-down→error, single-host-5xx-relayed) + per-host-cap tests (WithHosts cap queues; failover releases the primary slot) + transport-penalty tests (disabled keeps the keyless credential usable / default sidelines) + `internal/server` route tests, all `-race` — 2026-07-07.
...
- Each event captures: time, client (`config` for a static key, `localhost` for loopback, else the managed key name), IP (honouring `X-Forwarded-For` first hop behind a proxy), User-Agent, endpoint path, **provider** (resolved from the served model), model, upstream credential (account), input/output tokens, computed cost, and an error flag.
- `GET /admin/requests` (admin-authed) returns events newest-first as JSON with `count`, filter `total`, `offset`, and `total_cost`. Filters: `?model=` `?provider=` `?credential=` `?client=`; paginate with `?limit=` (default 200) + `?offset=`. The dashboard shows a bottom **"recent requests"** section: last 10 by default, "load more" endless pagination, and model/provider/account filter dropdowns (User-Agent/IP HTML-escaped); auto-refresh leaves the list alone while the operator is paginating or filtering.
...
**Verified:** `internal/usage` reqlog tests (ring/order, model/provider/credential filter, pagination offset+limit+total, default-cap) + `internal/server` tests (captures client/IP/UA/model/tokens/cost via /admin/requests, model filter) — 2026-06-12.
